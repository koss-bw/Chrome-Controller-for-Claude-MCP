// Shared runtime for chrome-controller-mcp tools.
//
...
const pidfilePath = path.join(
  os.tmpdir(),
  `chrome-controller-mcp-mcp-${TCP_PORT}.pid`
);
      ...
      if (!nativeHostSocket || nativeHostSocket.destroyed) {
        ...
        waitForNativeHost(5000).then((ok) => {
          ...
          if (ok && nativeHostSocket && !nativeHostSocket.destroyed) {
            ...
            reject(
              new Error(
                "Browser extension is not connected. Make sure a supported Chromium browser is running with the Chrome Controller for Claude extension installed and enabled."
              )
            );
          }
        });
...
export function coerceArgs(args) {
  ...
  // The api_* tools take arrays and objects, and a client that stringifies them
  // would otherwise fail zod validation with nothing to point at. Scalars are
  // deliberately left alone — zod coerces nothing, but every tool here accepts
  // them as numbers already.
  for (const key of ["resourceTypes", "headers", "patterns"]) {
